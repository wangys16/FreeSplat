...
class DatasetScannet(Dataset):
    ...
    def index(self) -> dict[str, Path]:
        ...
        print('-'*20 + f'view_sampler: {self.view_sampler}, data_stage: {self.data_stage},\
               isinstance: {isinstance(self.view_sampler, ViewSamplerEvaluation)}')
        if isinstance(self.view_sampler, ViewSamplerEvaluation) and self.data_stage == 'test':
