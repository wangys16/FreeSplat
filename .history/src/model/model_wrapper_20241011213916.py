...
class ModelWrapper(LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        if not test_fvs:
            ...
            psnr, lpips, ssim, masks = compute_metrics(batch["target"]["image"][0], output.color[0], 
                                                       use_mask=self.dataset_name == 'scannet', 
                                                       pre_mask=pre_mask)
            valid = masks.sum()
            if pre_mask is None:
            ...
            psnr_inter, lpips_inter, ssim_inter, masks_inter = compute_metrics(batch["target"]["image"][0][:length-fvs_length], 
                                                                  output.color[0][:length-fvs_length], use_mask=self.dataset_name == 'scannet',
    ...
    def validation_step(self, batch, batch_idx):
        ...
        psnr, lpips, ssim, masks = compute_metrics(rgb_gt, rgb, use_mask=self.dataset_name == 'scannet')
        valid = masks.sum()
