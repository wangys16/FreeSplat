...
def compute_metrics(rgb_gt, rgb, use_mask=False, pre_mask=None):
    ...
    if pre_mask is not None:
        mask = torch.tensor(pre_mask, device=rgb.device, dtype=torch.bool)
    ...
    psnr = (psnr * mask).sum() / (mask.sum()+1e-6)
    lpips = (lpips * mask).sum() / (mask.sum()+1e-6)
    ssim = (ssim * mask).sum() / (mask.sum()+1e-6)
    print('psnr:', psnr, 'lpips:', lpips, 'ssim:', ssim, 'mask:', mask.sum())
    return psnr, lpips, ssim, mask
...
class ModelWrapper(LightningModule):
    ...
    def __init__(
        ...
        self.loss_total = []
        self.metrics = {}
        for metric in ['psnr', 'lpips', 'ssim', 'delta_25']:
            self.metrics[metric] = []
        self.valids = []
        self.masks = {}
    ...
    def test_step(self, batch, batch_idx):
        ...
        abs_diff, rel_diff, delta_25 = depth_render_loss(output, batch)
        ...
        if not test_fvs:
            ...
            psnr, lpips, ssim, masks = compute_metrics(batch["target"]["image"][0], output.color[0], 
                                                       use_mask=self.dataset_name in ['scannet','replica'], 
                                                       pre_mask=pre_mask)
            valid = masks.sum()
            if pre_mask is None:
                self.masks[scene] = masks.tolist()
            ...
            psnr_inter, lpips_inter, ssim_inter, masks_inter = compute_metrics(batch["target"]["image"][0][:length-fvs_length], 
                                                                  output.color[0][:length-fvs_length], use_mask=self.dataset_name in ['scannet','replica'])
            psnr_extra, lpips_extra, ssim_extra, masks_extra = compute_metrics(batch["target"]["image"][0][length-fvs_length:],
                                                                  output.color[0][length-fvs_length:], use_mask=self.dataset_name in ['scannet','replica'])
            valid_inter, valid_extra = masks_inter.sum(), masks_extra.sum()
    ...
    def on_test_end(self) -> None:
        ...
        filename = 'test_idx.txt'
        if self.dataset_name in ['scannet', 'replica']:
            if 'scannet' in self.dataset_name:
                ...
                elif os.path.exists(f'/dataset/yswang/data/scannet/scannet/{filename}'):
                    path = '/dataset/yswang/data/scannet/scannet'
            with open(f'{path}/{filename}', 'r') as f:
                scenes = f.read().splitlines()
        if 'scannet' in self.dataset_name and not os.path.exists(f'assets/evaluation_index_scannet_{self.num_context_views}views_mask.json'):
            json.dump(self.masks, open(f'assets/evaluation_index_scannet_{self.num_context_views}views_mask.json', 'w'))
        for i in range(min(len(scenes), len(self.test_fvs_list))):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        psnr, lpips, ssim, masks = compute_metrics(rgb_gt, rgb, use_mask=self.dataset_name in ['scannet','replica'], )
        valid = masks.sum()
        self.log(f"val/psnr_{tag}", psnr)
        self.log(f"val/lpips_{tag}", lpips)
        self.log(f"val/ssim_{tag}", ssim)
        abs_diff, rel_diff, delta_25 = depth_render_loss(output_probabilistic, batch)
        ...
        sign = 1
        for metric in ['psnr', 'lpips', 'ssim', 'delta_25'] if sign else ['psnr', 'lpips', 'ssim']:
        ...
        if not self.train_cfg.depth_sup:
            ...
            for fig in batch["context"]["image"][0]:
                context_figs.append(fig)
            for s in range(-1,0):
                length = len(encoder_probabilistic_results[f"depth_num0_s{s}"][0])
                for i in range(length):
                    try:
                        context_depth_figs.append(torch.from_numpy(convert_array_to_pil(mmcv.imresize(batch["context"][f"depth_s{s}"][0][i][0].cpu().numpy(), (w,h),interpolation='nearest')).transpose(2,0,1)\
                                                                .astype(np.float32)/255).to(batch["context"]["image"][0].device))
                    except:
                        pass
                    try:
                        pred_depth_figs.append(torch.from_numpy(convert_array_to_pil(encoder_probabilistic_results[f"depth_num0_s{s}"][0][i].cpu().numpy().reshape(h,w)).transpose(2,0,1)\
                                                            .astype(np.float32)/255).to(batch["context"]["image"][0].device))
                    except:
                        pred_depth_figs.append(torch.from_numpy(convert_array_to_pil(mmcv.imresize(encoder_probabilistic_results[f"depth_num0_s{s}"][0][i].cpu().numpy().reshape(h//(2**(s+1)), w//(2**(s+1))), (w,h),interpolation='nearest'))\
