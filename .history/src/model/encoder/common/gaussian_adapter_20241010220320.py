...
class Create_from_depth_map():
    ...
    def __init__(self, intrinsic_matrix, height=960, width=1280, depth_trunc=12.0):
        ...
        self.one_mat = torch.ones((height, width, 1)).to(intrinsic_matrix.device)  # This one_mat is to build the 4_d vector (x, y, z, 1) which represents a point in the camera coordinate
        #
        ...
        # print('width:', width)
        # print('height:', height)
        # print('coord_mat:', coord_mat.reshape(-1,2))
        # print('self.cam_coord_part:', self.cam_coord_part)
    ...
    def project(self, depth_map, extrinsic_c2w_matrix):
        ...
        # print('world_coord_mat1:', world_coord_mat1)
        ...
        # xy_coords = cam_coord_xyz.permute(2, 0, 1).reshape(3, self.height * self.width)
        # xy_coords = (xy_coords / xy_coords[2:3, :])[:2]
        ...
        # print('post 2d_coords:', post_xy_coords)
        # exit(0)
...
class GaussianAdapter(nn.Module):
    ...
    def forward(
        ...
        # if load_depth:
        #     mask = (depths > 0) * (depths < 15)
        #     opacities[~mask] = 0
        
        if coords is None:
            if self.cfg.depth_sup:
                # print('USING DEPTH!!!!!!!!!!!!!')
                ...
                for i in range(b):
                    ...
                    depth_to_pcd_now = Create_from_depth_map(intrinsic, height=h, width=w,
                                                    depth_trunc=15)
                    for j in range(v):
                        pcd_w1_now, pcd_dir1, point_mask1 = depth_to_pcd_now.project(depths[i,j].view(h,w),\
                                                                                    extrinsics[i,j].view(4,4))
                        new_means[i,j] = pcd_w1_now[:,None,None]
                
                # print('(new_means - means).abs().mean():', (new_means - means).abs().mean())
                means = new_means
