...
from ..encoder_freesplat import EncoderFreeSplat
...
class EncoderVisualizerEpipolar(
    EncoderVisualizer[EncoderVisualizerEpipolarCfg, EncoderFreeSplat]
):
    def visualize(
        self,
        result,
        ...
        visualization_dump = result['visualizations']
        result = result['gaussians']
        ...
        if self.cfg.export_ply and out_path is not None:
            name = wandb.run._name.split(" ")[0]
            ply_path = Path(f"{out_path}/{global_step:0>6}.ply")
            print('saving .ply to path:', ply_path)
            # print('scales:', visualization_dump["scales"])
            export_ply(
                context["extrinsics"][0, 0],
                result.means[0],
                visualization_dump["scales"][0]*2,
                visualization_dump["rotations"][0],
                result.harmonics[0],
                torch.logit(result.opacities[0]),
                ply_path,
            )
        try:
            return {
