...
class EpipolarSampler(nn.Module):
    ...
    def forward(
        ...
        # print('samples range:', samples.min()[0], samples.max()[0])
        ...
        samples = rearrange(
            samples, "(b v) c (ov r s) () -> b v ov r s c", b=b, v=v, ov=min(2,v - 1), s=s
        )
    ...
    def transpose(
        ...
        # print(f't_b.range: {t_b.min()} {t_b.max()}')
        # print(f't_v.range: {t_v.min()} {t_v.max()}')
        # print(f't_ov.range: {t_ov.min()} {t_ov.max()}')
        # print(f't_v: {t_v}, t_ov: {t_ov}')
    ...
    def collect(
        ...
        index_b = repeat(index_b, "b -> b v ov", v=v, ov=index_v.shape[2])
