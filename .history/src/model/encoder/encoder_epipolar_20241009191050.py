    ...
    def forward(
        ...
        if self.cfg.backbone.name == 'dino':
            ...
            for num in range(length):
                ...
                xy_ray, _ = sample_image_grid((h, w), device)
                gaussians_feats = rearrange(depth_outputs[f'output_pred_s-1_b1hw'][:,1:], '(b v) c h w -> b v h w c', b=b, v=n_views)#.view(b,n_views,h//(2**(s+1)), w//(2**(s+1)),64)
                ...
                # try:
                results[f'depth_num{num}_s-1'] = depths
                try:
                    depths_raw = rearrange(context[f'depth_s-1'], "b v c h w -> b v (h w) c 1")
                    results[f'depth_num{num}_s-1_raw'] = depths_raw
                    mask = (depths_raw > 1e-3) * (depths_raw < 10)
                    results[f'depth_num{num}_s-1_mask'] = mask
                except:
                    pass
                results[f'depth_num{num}_s-1_b1hw'] = depth_outputs[f'depth_pred_s-1_b1hw']
