        ...
        if self.use_planes:
            ...
            if self.refine:
                fine_feat = F.interpolate(
                                depth_outputs[f"output_pred_s0_b1hw"],
                                scale_factor=2,
                                mode="bilinear",
                                align_corners=True,
                            )
                disps_delta = self.refine_unet(torch.cat([fine_disps, fine_feat], dim=1))
                fine_disps = (fine_disps + disps_delta).clamp(
                            1.0 / self.far,
                            1.0 / self.near,
                        )
            depth_map = torch.exp(fine_disps) if self.log_plane else 1.0 / fine_disps
