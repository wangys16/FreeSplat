...
from src.model.encoder.attention.transformer import LocalFeatureTransformer
...
class CostVolumeManager(nn.Module):
    ...
    def forward(
        ...
        # try:
        #     cost_volume, depth_planes_bdhw, overall_mask_bhw, coords, src_feat_warped, dot_results = \
        ...
        #                                 depth_planes_bdhw,
        #                             )
        #     return cost_volume, lowest_cost, depth_planes_bdhw, overall_mask_bhw, coords, src_feat_warped, dot_results
        # except:
...
class AttentionVolumeManager(CostVolumeManager):
    ...
    def __init__(
        ...
        self.transformer = LocalFeatureTransformer(d_model=16, nhead=4, layer_names=['self'], attention='linear')


        self.mlp = MLP(channel_list=[1+16,64,64,1], disable_final_activation=True)
        # self.viewToken = ViewTokenNetwork(dim=16)
    ...
    def build_cost_volume(
        ...
        for depth_id in range(self.num_depth_bins):
            ...
            # print('src_feat_warped.shape:', src_feat_warped.shape)                  # [3, 2, 16, 96, 128]
            # print('cur_feats.unsqueeze(1).shape:', cur_feats.unsqueeze(1).shape)    # [3, 1, 16, 96, 128]
            # print('dot_product_b1hw.shape:', dot_product_b1hw.shape)                # [3, 1, 96, 128]

            x = torch.cat([cur_feats.unsqueeze(1), src_feat_warped], axis=1)
            B, V, C, H, W = x.shape
            x = rearrange(x, "B V C H W -> (B H W) V C")
            x = self.transformer(x)
            x1 = rearrange(x, "(B H W) V C -> B V C H W", B=B, H=H, W=W)
            x = x1[:,0]

            dot_product_b1hw = rearrange(torch.cat([dot_product_b1hw, x], axis=1), "B C H W -> B H W C")
            dot_product_b1hw = self.mlp(dot_product_b1hw)
            dot_product_b1hw = rearrange(dot_product_b1hw, "B H W C -> B C H W")
...
class AVGFeatureVolumeManager(CostVolumeManager):
    ...
    def build_cost_volume(self, 
        ...
        for depth_id in range(self.num_depth_bins):
            ...
            # print('src_feats.shape:', src_feats.view(
            #                                             -1, 
            #                                             num_feat_channels, 
            #                                             self.matching_height, 
            #                                             self.matching_width
            #                                         ).shape, 'uv_coords.shape:', uv_coords.shape)
            ...
            # print('warped_ratio:', (src_feat_warped!=0).sum()/src_feat_warped.numel())
            ...
            #                             3,
            #                         )
            # print(f'src_feat_warped.shape:', src_feat_warped.shape)
            # print(f'dot_product_bkhw.shape:', dot_product_bkhw.shape)
            ...
            # combined_visual_features_bchw = src_feat_warped.mean(dim=1)
            ...
            # print('mlp_input_features_bhwc.shape:', mlp_input_features_bhwc.shape)
