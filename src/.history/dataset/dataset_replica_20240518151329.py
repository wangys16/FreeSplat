...
from .dataset_re10k import DatasetRE10kCfg as DatasetReplicaCfg
...
class DatasetReplica(Dataset):
    ...
    def __init__(
        ...
        if not os.path.exists(cfg.roots[0]):
            ...
            elif os.path.exists('/home/wang/ssd/replica/ps'):
                cfg.roots[0] = Path('/home/wang/ssd/replica/ps')
    ...
    def __getitem__(self, idx):
        ...
        # if not self.data_stage == 'test_fvs':
        #     context_indices = [context_indices]
        # print('raw intrinsics:', intrinsics[context_indices])
        # print('imshape:', imshape[2], imshape[1])
        # print('raw intrinsics:', intrinsics)
        ...
        if not test_fvs:
            ...
            length = len(target_indices)
            # extrinsics_1 = np.load(os.path.join(str(path).replace(f'_0{sign}', f'_0{1-sign}'), 'extrinsics.npy'))
            # transformation = np.load(os.path.join(str(path).replace(f'_0{sign}', f'_0{1-sign}'), 'transformation.npy'))
            # x = torch.tensor([np.dot(transformation, x) for x in extrinsics_1]).float()
