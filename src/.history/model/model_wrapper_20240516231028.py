...
class ModelWrapper(LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        loss_depth_render = depth_render_loss(output, batch)
        self.benchmarker.store('loss_depth_render', float(loss_depth_render.detach().cpu().numpy()))
        try:
        ...
        for i, depth_render, depth_gt in zip(range(len(batch["context"][0]["index"][0])), output.depth[0][0], batch["target"]['depth'][0,:,0]):
            save_image(torch.from_numpy(convert_array_to_pil(depth_render[2:h-2,2:w-2], no_text=True).transpose(2,0,1)\
                                            .astype(np.float32)/255).to(batch["context"][0]["image"][0].device),
                                            path / scene / f"depth_render/{index:0>6}.png")
            save_image(torch.from_numpy(convert_array_to_pil(depth_gt[2:h-2,2:w-2], no_text=True).transpose(2,0,1)\
                                            .astype(np.float32)/255).to(batch["context"][0]["image"][0].device),
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss_depth = depth_render_loss(encoder_probabilistic_results, batch)
        self.log(f"val/loss_depth_{tag}", loss_depth)
        sign = 1
        # except:
        #     sign = 0
