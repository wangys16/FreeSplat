...
def warp_with_pose_depth_candidates(
    ...
    with torch.no_grad():
        ...
        mask = torch.zeros([w, h], device=depth.device).bool()
        indices = pixel_coords[0,:,15].permute(1,0).type(torch.int64)
        rows = indices[:, 0]
        cols = indices[:, 1]
        values = torch.ones_like(rows, dtype=torch.bool)
        mask.scatter_(0, rows.unsqueeze(0), values.unsqueeze(0), reduce="add")
        mask.scatter_(1, cols.unsqueeze(0), values.unsqueeze(0), reduce="add")
    ...
    # print('grid:', grid.size(), 'feature1:', feature1.size())
    # sample features
...
class DepthPredictorMultiView(nn.Module):
    ...
    def forward(
        ...
        # print('depth_candidates:', 1 / disp_candi_curr)
