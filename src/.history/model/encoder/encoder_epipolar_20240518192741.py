                ...
                for s in range(-1, self.cfg.n_levels+1):
                    ...
                    if not export_ply:
                        ...
                        if not resized:
                            ...
                            h = h - 8
                            w = w - 8
                            context['image_shape'] = (context['image_shape'][0]-4, context['image_shape'][1]-4)
                            resized = 1
                        xy_ray, _ = sample_image_grid((h, w), device)
                        xy_ray = xy_ray + torch.tensor([[[2]]], dtype=torch.float32, device=device)
                        depth_outputs[f'output_pred_s{s}_b1hw'] = depth_outputs[f'output_pred_s{s}_b1hw'][:, :, 4:-4, 4:-4]
                        depth_outputs[f'depth_pred_s{s}_b1hw'] = depth_outputs[f'depth_pred_s{s}_b1hw'][:, :, 4:-4, 4:-4]
                        depth_outputs[f'depth_weights'] = depth_outputs[f'depth_weights'][:, :, 4:-4, 4:-4]
                        skip = skip[:, :, 4:-4, 4:-4]
                    # print('h, w:', h, w)
