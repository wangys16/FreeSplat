            ...
            for num in range(length):
                ...
                for s in range(-1, self.cfg.n_levels+1):
                    ...
                    if not export_ply:
                        ...
                        depth_outputs[f'output_pred_s{s}_b1hw'] = depth_outputs[f'output_pred_s{s}_b1hw'][:, :, 2:-2, 2:-2]
                        depth_outputs[f'depth_pred_s{s}_b1hw'] = depth_outputs[f'depth_pred_s{s}_b1hw'][:, :, 2:-2, 2:-2]
                        depth_outputs[f'depth_weights'] = depth_outputs[f'depth_weights'][:, :, 2:-2, 2:-2]
                        skip = skip[:, :, 2:-2, 2:-2]
                    # print('h, w:', h, w)
