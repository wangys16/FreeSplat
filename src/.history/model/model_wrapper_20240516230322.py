...
class ModelWrapper(LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        loss_depth = depth_render_loss(output)
        try:
        ...
        for i, depth_render, depth_gt in zip(range(len(batch["context"][0]["index"][0])), output.depth[0][0], batch["target"]['depth'][0,:,0]):
            save_image(torch.from_numpy(convert_array_to_pil(save_image(torch.from_numpy(convert_array_to_pil(encoder_results[f"depth_num0_s{s}"][0][i].cpu().numpy().reshape(h,w)[2:h-2,2:w-2], no_text=True).transpose(2,0,1)\
                                            .astype(np.float32)/255).to(batch["context"][0]["image"][0].device),
                                            path / scene / f"depth_pred/{index:0>6}.png")[2:h-2,2:w-2], no_text=True).transpose(2,0,1)\
                                            .astype(np.float32)/255).to(batch["context"][0]["image"][0].device),
                                            path / scene / f"depth_pred/{index:0>6}.png")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss_depth = depth_render_loss(encoder_probabilistic_results)
