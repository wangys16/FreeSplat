...
def depth_render_loss(prediction, batch) -> Float[Tensor, ""]:
    ...
    # print(f'target.shape: {target.shape}, prediction.depth.shape: {prediction.depth.shape}')
    ...
    # print('abs_diff_b.shape:', abs_diff_b.shape)
    ...
    def on_test_end(self) -> None:
        ...
        print('psnr_inter_avg:', (np.array(self.benchmarker.benchmarks['psnr_inter']) 
            ...
            'lpips_inter_avg:', (np.array(self.benchmarker.benchmarks['lpips_inter']) 
                                * np.array(self.benchmarker.benchmarks['valid_inter'])).sum()
                                / np.array(self.benchmarker.benchmarks['valid_inter']).sum(),
            'depth_abs_diff_avg:', torch.nanmean(self.benchmarker.benchmarks['depth_abs_diff']).cpu().numpy(),
            'depth_rel_diff_avg:', torch.nanmean(self.benchmarker.benchmarks['depth_rel_diff']).cpu().numpy(),
            'depth_delta_25_avg:', torch.nanmean(self.benchmarker.benchmarks['depth_delta_25']).cpu().numpy())
        try:
