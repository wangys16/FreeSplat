...
def project_rays(
    ...
    for min_valid, max_valid in itertools.product([True, False], [True, False]):
        ...
        result["xy_min"][mask] = min_value["xy"][mask].type_as(result["xy_min"])
        result["xy_max"][mask] = max_value["xy"][mask].type_as(result["xy_max"])
        result["overlaps_image"][mask] = (min_value["valid"] & max_value["valid"])[mask].type_as(result["overlaps_image"])
